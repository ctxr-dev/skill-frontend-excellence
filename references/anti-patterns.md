...
load-when:
  task-keywords: [anti-pattern, what to avoid, mistake, audit, review, polish]
...
|--------|---------|--------------|-----------|
| Hardcoded asset path that breaks on hash change | The next deploy invalidates every preload at once | Preload by emitted URL, resolved at build time | [performance.md](performance.md) |
...
| Modal that traps Tab but not screen reader | Background remains in the AT tree | Mark every sibling of the dialog `inert` while open | [accessibility.md](accessibility.md), [debug-recipes.md](debug-recipes.md) |
...
| Fabricated structured data (claims a review or rating that does not exist) | Manual action from Google; removed from rich results | Only mark up content the user actually sees | [seo.md](seo.md) |
| `noindex` page still listed in `sitemap.xml` | Wastes crawl budget; mixed signals | Remove from sitemap; the sitemap is the indexable index | [seo.md](seo.md) |
...
| robots.txt blocks JS or CSS the page needs to render | Crawler renders broken page; ranking drops | Allow JS and CSS routes; only block sensitive paths | [seo.md](seo.md) |
...
| Hover-only menu on touch | First tap opens, second tap navigates; users get stuck | Show on both hover and focus and tap; or use a press-to-open menu | [ui-ux.md](ui-ux.md) |
| Modal with no Esc handler | Keyboard users have no way out | Esc closes; focus restored to trigger | [ui-ux.md](ui-ux.md), [accessibility.md](accessibility.md) |
| Custom dropdown that does not support arrow keys | Keyboard users cannot select | Implement the WAI-ARIA combobox or listbox pattern in full | [ui-ux.md](ui-ux.md), [accessibility.md](accessibility.md) |
...
| Pattern | Why bad | Standard fix | Deep link |
|--------|---------|--------------|-----------|
| Inter on white with a purple gradient | The default AI aesthetic; instantly recognised as generic | Pick a typeface with character (a serif, a humanist sans, a variable display); pair with a restrained palette | [design.md](design.md) |
...
| Brand accent that fails contrast on dark mode | Brand recognition costs accessibility | Compute the dark-mode brand variant against the dark surface; aim for 4.5:1 on text, 3:1 on UI | [design.md](design.md), [accessibility.md](accessibility.md) |
...
| Component without a Storybook (or equivalent playground) entry | No way to reproduce in isolation; bug reports route to the page that hosts it | Every shared component has a playground entry per variant | [components.md](components.md) |
...
| No `autocomplete` on a sign-up form | Password managers and autofill do not work | Every field has the WHATWG `autocomplete` value | [forms.md](forms.md) |
...
| Pattern | Why bad | Standard fix | Deep link |
|--------|---------|--------------|-----------|
| Body scroll lock without `padding-right` compensation | Page jumps sideways when modal opens | Measure scrollbar width; set `body { padding-right: <gap> }` while locked | [defects.md](defects.md), [ui-ux.md](ui-ux.md) |
...
| Focus ring clipped by `overflow: hidden` | Keyboard users cannot see focus | Move ring to `outline` (paints outside) or add internal padding | [accessibility.md](accessibility.md), [defects.md](defects.md) |
